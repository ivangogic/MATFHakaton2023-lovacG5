...
class Compound(Node):
    ...
    def to_json(self):
        json_list = [
            item.to_json() for item in self.block_items
        ]
        return json_list
...
class Decl(Node):
    ...
    def to_json(self):
        num_stars = 0
        current = self.type
        while current.__class__.__name__ == 'PtrDecl':
            num_stars += 1
            current = current.type
        if not current.__class__.__name__ == 'TypeDecl':
            return ''
        json_dict = {
            'class': 'declare',
            'name': self.name,
...
class FileAST(Node):
    ...
    def to_json(self):
        json_list = [
            item.to_json() for item in self.ext
        ]
        return json_list
